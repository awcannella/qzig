...
// =====================================================
//
//   zig test src/experimental/simd_executor.zig
//
// Tests compare cache-blocked output against scalar reference
// amplitude-by-amplitude at 1e-12 tolerance.
// The H²=I involution tests are the most important — they catch
// accumulated floating-point error and wrong index arithmetic.

const testing = std.testing;

fn scalar_h_reference(sv: *StateVector, targets: []const usize) void {
    ...
    for (targets) |q| {
        ...
        while (base < len) : (base += step) {
            var j: usize = 0;
            while (j < stride) : (j += 1) {
                butterfly_scalar(sv, base + j, base + j + stride, inv);
            }
        }
    }
}

fn expect_sv_equal(a: *StateVector, b: *StateVector) !void {
    for (0..a.len()) |i| {
        try testing.expectApproxEqAbs(a.re[i], b.re[i], 1e-12);
        try testing.expectApproxEqAbs(a.im[i], b.im[i], 1e-12);
    }
}

test "blocked H — 4 qubits all targets (small stride)" {
    const alloc = testing.allocator;
    var ref = try StateVector.init_zero(alloc, 4);
    defer ref.deinit(alloc);
    var blk = try StateVector.init_zero(alloc, 4);
    defer blk.deinit(alloc);
    const t = [_]usize{ 0, 1, 2, 3 };
    scalar_h_reference(&ref, &t);
    execute_h_blocked(&blk, .{ .targets = &t });
    try expect_sv_equal(&ref, &blk);
}

test "blocked H — 8 qubits all targets (small stride)" {
    const alloc = testing.allocator;
    var ref = try StateVector.init_zero(alloc, 8);
    defer ref.deinit(alloc);
    var blk = try StateVector.init_zero(alloc, 8);
    defer blk.deinit(alloc);
    const t = [_]usize{ 0, 1, 2, 3, 4, 5, 6, 7 };
    scalar_h_reference(&ref, &t);
    execute_h_blocked(&blk, .{ .targets = &t });
    try expect_sv_equal(&ref, &blk);
}

test "blocked H — 10 qubits all targets (at threshold q=10)" {
    const alloc = testing.allocator;
    var ref = try StateVector.init_zero(alloc, 10);
    defer ref.deinit(alloc);
    var blk = try StateVector.init_zero(alloc, 10);
    defer blk.deinit(alloc);
    var t: [10]usize = undefined;
    for (&t, 0..) |*v, i| v.* = i;
    scalar_h_reference(&ref, &t);
    execute_h_blocked(&blk, .{ .targets = &t });
    try expect_sv_equal(&ref, &blk);
...
test "blocked H — 14 qubits all targets (crosses large-stride)" {
    const alloc = testing.allocator;
    var ref = try StateVector.init_zero(alloc, 14);
    defer ref.deinit(alloc);
    var blk = try StateVector.init_zero(alloc, 14);
    defer blk.deinit(alloc);
    var t: [14]usize = undefined;
    for (&t, 0..) |*v, i| v.* = i;
    scalar_h_reference(&ref, &t);
    execute_h_blocked(&blk, .{ .targets = &t });
    try expect_sv_equal(&ref, &blk);
...
test "blocked H — 18 qubits all targets (deep large-stride)" {
    const alloc = testing.allocator;
    var ref = try StateVector.init_zero(alloc, 18);
    defer ref.deinit(alloc);
    var blk = try StateVector.init_zero(alloc, 18);
    defer blk.deinit(alloc);
    var t: [18]usize = undefined;
    for (&t, 0..) |*v, i| v.* = i;
    scalar_h_reference(&ref, &t);
    execute_h_blocked(&blk, .{ .targets = &t });
    try expect_sv_equal(&ref, &blk);
...
test "blocked H — single high qubit q=15 (pure large-stride)" {
    const alloc = testing.allocator;
    var ref = try StateVector.init_zero(alloc, 16);
    defer ref.deinit(alloc);
    var blk = try StateVector.init_zero(alloc, 16);
    defer blk.deinit(alloc);
    const t = [_]usize{15};
    scalar_h_reference(&ref, &t);
    execute_h_blocked(&blk, .{ .targets = &t });
...
test "blocked H — single low qubit q=0 (pure scalar fallback)" {
    const alloc = testing.allocator;
    var ref = try StateVector.init_zero(alloc, 16);
    defer ref.deinit(alloc);
    var blk = try StateVector.init_zero(alloc, 16);
...
test "H²=I involution — 14 qubits" {
    // Apply H twice: result must recover |0⟩ exactly.
    const alloc = testing.allocator;
    const nq = 14;
    var sv = try StateVector.init_zero(alloc, nq);
    defer sv.deinit(alloc);
    var t: [nq]usize = undefined;
    for (&t, 0..) |*v, i| v.* = i;
    const h = KernelBlock.Hadamard{ .targets = &t };
    execute_h_blocked(&sv, h);
    execute_h_blocked(&sv, h);
    try testing.expectApproxEqAbs(@as(f64, 1.0), sv.re[0], 1e-10);
    for (1..sv.len()) |i| {
        try testing.expectApproxEqAbs(@as(f64, 0.0), sv.re[i], 1e-10);
        try testing.expectApproxEqAbs(@as(f64, 0.0), sv.im[i], 1e-10);
    }
}

test "H²=I involution — 18 qubits" {
    const alloc = testing.allocator;
    const nq = 18;
    var sv = try StateVector.init_zero(alloc, nq);
    defer sv.deinit(alloc);
    ...
    execute_h_blocked(&sv, h);
    execute_h_blocked(&sv, h);
    try testing.expectApproxEqAbs(@as(f64, 1.0), sv.re[0], 1e-8);
    for (1..sv.len()) |i| {
        try testing.expectApproxEqAbs(@as(f64, 0.0), sv.re[i], 1e-8);
        try testing.expectApproxEqAbs(@as(f64, 0.0), sv.im[i], 1e-8);
    }
}
